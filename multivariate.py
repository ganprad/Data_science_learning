...
def load_dataset(file_loc, **kwargs):
    ...
    df = pd.read_csv(
        z.open('LoanStats3d.csv'), skiprows=[0], nrows=kwargs['nrows'])
    return df


def preprocess(df):

    df['logannual_inc'] = df['annual_inc'].map(
        lambda x: np.log1p(np.float(str(x))))
    df['annual_inc'] = df['annual_inc'].map(lambda x: np.float(str(x)))
    df['int_rate'] = df['int_rate'].map(
        lambda x: np.float(str(x).strip().split('%')[0]))
    df['logint_rate'] = df['int_rate'].map(
        lambda x: np.log1p(np.float(str(x).strip().split('%')[0])))
    return df
...
if __name__ == '__main__':
    ...
    param = est.params
    income_linspace = np.linspace(
        df.logannual_inc.min(), df.logannual_inc.max(), 100)

    plt.scatter(df.logannual_inc, df.int_rate, alpha=0.3)
    plt.xlabel('LogIncome')
    plt.ylabel('InterestRate')
    plt.title(formula)
    plt.axis([np.min(df.logannual_inc), np.max(df.logannual_inc),
              np.min(df.int_rate), np.max(df.int_rate)])
    # OWN
